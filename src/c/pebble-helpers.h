...
//Safely set a bitmap
void bitmap_layer_set_bitmap_safe(BitmapLayer *layer, GBitmap *bitmap, uint32_t resource_id);

//Safely destroy a bitmap
void gbitmap_destroy_safe(GBitmap *bitmap);

// Safely mak dirty a layer
void layer_mark_dirty_safe(Layer *layer);

// Safely cancel a timer
void app_timer_cancel_safe(AppTimer *timer);